use crate::maps::components::{MapBackgroundAnimatedTile, MapBackgroundTilemap};
use bevy::prelude::*;
...
pub fn setup_map_background(
    ...
    active_map: Res<crate::maps::components::ActiveMap>,
    windows: Query<&Window, With<PrimaryWindow>>,
) {
    let config = &active_map.config;
    ...
    let (tiles_x, tiles_y) = if let Ok(window) = windows.single() {
        (
            (window.width() / pixels_per_tile).ceil() as u32 + config.bg_viewport_padding_tiles * 2,
